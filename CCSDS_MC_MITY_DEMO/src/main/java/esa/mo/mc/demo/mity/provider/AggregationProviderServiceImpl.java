...
public class AggregationProviderServiceImpl extends AggregationInheritanceSkeleton
{
  ...
  { // requirement 3.7.6.2.1
    ...
    for (int index = 0; index < lLongList.size(); index++)
    {
      ...
      if (tempLong == 0)
      {  // Is it the wildcard '0'? requirement: 3.7.6.2.2
      ...
      if (tempPDef != null)
      { // Does the AggregationDefinition exist?
        outLongLst.add(tempLong); //yap
      }
      else
      {  // The requested aggregation is unknown
  ...
  { // requirement: 3.7.7.2.1
    ...
    for (int index = 0; index < lInstanceBooleanPairList.size(); index++)
    {
      ...
      if (tempBPair.getId() == 0)
      {  // Is it the wildcard '0'? requirement: 3.7.7.2.4
      ...
      {  // requirement: 3.7.7.2.8 (is respected because no error is generated if it is already enabled)
  ...
  { // requirement: 3.7.8.2.1
    ...
    for (int index = 0; index < lInstanceBooleanPairList.size(); index++)
    {
      ...
      if (tempBPair.getId() == 0)
      {  // Is it the wildcard '0'? requirement: 3.7.8.2.4
      ...
      { // requirement: 3.7.8.2.8 (is respected because no error is generated if it is already enabled)
  ...
  { // requirement: 3.7.9.2.1
    ...
    for (Identifier tempIdentifier : lIdentifier)
    {
      ...
      if (tempIdentifier.toString().equals("*"))
      {  // requirement: 3.7.9.2.2
        outLongLst.clear();  // if the wildcard is in the middle of the input list, we clear the output list and...
        outLongLst.addAll(manager.listAll()); // ... add all in a row
        break;
      }
                                ...
                                final MALInteraction interaction) throws MALException, MALInteractionException
  {
    ...
    for (int index = 0; index < lAggregationDefinitionList.size(); index++)
    { // requirement: 3.7.10.2.5 (incremental "for cycle" guarantees that)
      ...
      if (tempAggregationDefinition.getName() == null
              ...
              || tempAggregationDefinition.getName().equals(new Identifier("")))
      { // requirement: 3.7.10.2.2
        invIndexList.add(new UInteger(index));
      }

      if (manager.list(tempAggregationDefinition.getName()) == null)
      { // Is the supplied name unique? requirement: 3.7.10.2.3
  ...
  public void updateDefinition(final LongList lLongList, final AggregationDefinitionDetailsList lAggregationDefinitionList,
                               final MALInteraction interaction) throws MALException, MALInteractionException
  { // requirement: 3.7.11.2.1, 3.7.11.2.2, 3.7.11.2.3
    ...
    for (int index = 0; index < lLongList.size(); index++)
    {
      ...
      if (oldAggregationDefinition == null)
      { // The object instance identifier could not be found? // requirement: 3.7.11.2.5
        unkIndexList.add(new UInteger(index));
        continue;
      }

      if (tempAggregationDefinition.getName().equals(oldAggregationDefinition.getName()))
      { // Are the names equal? requirement: 3.7.11.2.6
  ...
  { // requirement: 3.7.12.2.1
    ...
    for (int index = 0; index < lLongList.size(); index++)
    {
      ...
      if (tempLong == 0)
      {  // Is it the wildcard '0'? requirement: 3.7.12.2.2
      ...
      if (manager.exists(tempLong))
      { // Does it match an existing definition? requirement: 3.7.12.2.3
  ...
  { // requirement: 3.7.2.1a
    ...
    public void refresh(Long objId)
    {
      ...
      if (ADef != null)
      { // Does it exist in the Aggregation Definitions List?
...
//        if (manager.get(objId).getFilterEnabled() && manager.get(objId).getFilteredTimeout().getValue() != (float) 0){ // requirement 3.7.2.12
      if (manager.get(objId).getFilterEnabled() && manager.get(objId).getFilteredTimeout().getValue() != 0)
      { // requirement 3.7.2.12
        ...
        {  // requirement: 3.7.2.3
          if (active)
          {
            ...
            {  // requirement 3.7.2.9
              ...
              if (def.getFilterEnabled() && manager.isFilterTriggered(objId) == true)
              { // The Filter is on and triggered? requirement: 3.7.2.6
    ...
    private void resetFilterTimeoutTimer(Long objId)
    {
      ...
      if (filterTimeoutTimerList.get(index) == null)
      {
        ...
        public void run()
        {
          if (active)
          {
            ...
            if (aggregationDefinition.getFilterEnabled() && // Is the filter enabled? 
                    ...
                    aggregationParameterSet.getParameters().size() == 1)
            {  // Are we sampling one parameter? requirement: 3.7.2.8
              ...
              for (int i = 0; i < currentParameterValue.size(); i++)
              {
                ...
                if (current != null && previous != null)
                {
                  ...
                  if ((current.getIsValid() && previous.getIsValid()) || // Are the parameters valid?
                          (current.getInvalidSubState().getValue() == 2 && previous.getInvalidSubState().getValue() == 2))
                  { // 2 stands for the INVALID_RAW state
