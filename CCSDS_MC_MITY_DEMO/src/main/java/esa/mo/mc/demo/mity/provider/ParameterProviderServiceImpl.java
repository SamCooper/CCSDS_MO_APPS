...
public class ParameterProviderServiceImpl extends ParameterInheritanceSkeleton
{
  ...
  { // requirement 3.3.6.2.1
    ...
    for (int index = 0; index < lLongList.size(); index++)
    {
      ...
      if (tempLong == 0)
      {  // Is it the wildcard '0'? requirement: 3.3.6.2.2
      ...
      if (tempPDef != null)
      { // Does the ParameterDefinition exist?
  ...
  { // requirement: 3.3.7.2.1
    ...
    for (int index = 0; index < lInstanceBooleanPairList.size(); index++)
    {
      ...
      {  // Is it the wildcard '0'? requirement: 3.3.7.2.4
                                ...
                                final MALInteraction interaction) throws MALException, MALInteractionException
  {
    ...
    for (int index = 0; index < lParameterDefinitionList.size(); index++)
    { // requirement: 3.3.9.2.5 (incremental "for cycle" guarantees that)
      ...
      if (tempParameterDefinition.getName() == null
              ...
              || tempParameterDefinition.getName().equals(new Identifier("")))
      { // requirement: 3.3.9.2.2
        invIndexList.add(new UInteger(index));
      }

      if (manager.list(tempParameterDefinition.getName()) == null)
      { // Is the supplied name unique? requirement: 3.3.9.2.3
  ...
  { // requirement: 3.3.10.2.1
    ...
    for (int index = 0; index < lLongList.size(); index++)
    {
      ...
      if (oldParameterDefinition == null)
      { // The object instance identifier could not be found? // requirement: 3.3.10.2.5
        unkIndexList.add(new UInteger(index));
        continue;
      }

      if (tempParameterDefinition.getName().equals(oldParameterDefinition.getName()))
      { // Are the names equal? requirement: 3.3.10.2.6
  ...
  { // requirement: 3.3.11.2.1
    ...
    for (int index = 0; index < lLongList.size(); index++)
    {
      ...
      if (tempLong == 0)
      {  // Is it the wildcard '0'? requirement: 3.3.11.2.2
      ...
      { // Does it match an existing definition? requirement: 3.3.11.2.3
...
//    DurationList updateIntervalList; //Update Interval List
    ...
    public void init()
    {   // Refresh all the Parameter Definitions on the Manager
...
//            if (PDef.getUpdateInterval().getValue() != (float) 0 && PDef.getGenerationEnabled() ) // Is the periodic reporting active?
    ...
    private void startTimer(final int index, final Duration interval)
    {  // requirement: 3.3.2.11
